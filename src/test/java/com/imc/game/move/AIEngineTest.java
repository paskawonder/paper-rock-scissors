...
class AIEngineTest {
    ...
    void consumeGameMovesGetNextMoveTest() {
        ...
        Mockito.when(messageQueue.readMsgs(MessageQueue.TOPIC_GAME_SINGLE_PLAYER_MOVE, PLAYER_ID)).thenReturn(List.of(
                new PlayerIdMove(PLAYER_ID, Move.ROCK), new PlayerIdMove(PLAYER_ID, Move.ROCK), new PlayerIdMove("1", Move.PAPER), new PlayerIdMove("2", Move.SCISSORS)
        ));
        subject.consumeGameMoves(PLAYER_ID);
        Assertions.assertSame(Move.SCISSORS, subject.getNextMove(PLAYER_ID));
        Mockito.when(messageQueue.readMsgs(MessageQueue.TOPIC_GAME_SINGLE_PLAYER_MOVE, PLAYER_ID)).thenReturn(List.of(
                new PlayerIdMove(PLAYER_ID, Move.ROCK), new PlayerIdMove(PLAYER_ID, Move.ROCK), new PlayerIdMove("2", Move.PAPER)
        ));
        subject.consumeGameMoves(PLAYER_ID);
        Assertions.assertSame(Move.PAPER, subject.getNextMove(PLAYER_ID));
        Mockito.when(messageQueue.readMsgs(MessageQueue.TOPIC_GAME_SINGLE_PLAYER_MOVE, PLAYER_ID)).thenReturn(List.of(
                new PlayerIdMove("1", Move.ROCK), new PlayerIdMove("1", Move.ROCK)
        ));
        subject.consumeGameMoves(PLAYER_ID);
        Assertions.assertSame(Move.PAPER, subject.getNextMove(PLAYER_ID));
        Mockito.when(messageQueue.readMsgs(MessageQueue.TOPIC_GAME_SINGLE_PLAYER_MOVE, PLAYER_ID)).thenReturn(List.of(
                new PlayerIdMove("1", Move.ROCK), new PlayerIdMove("2", Move.ROCK), new PlayerIdMove("3", Move.PAPER)
        ));
        subject.consumeGameMoves(PLAYER_ID);
